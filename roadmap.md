...
only the HTTPS authority and admits exactly the ESPN, NHL, and MLB base/www
hosts. Both `SourceLinkButton` and the bottom ticker pass the validated original
URL to the existing argument-array browser helper. Executable tests cover
case-insensitive HTTPS acceptance plus deceptive path, userinfo, suffix-domain,
port, malformed, non-HTTPS, and missing-value rejection.

All required gates pass: JavaScript suite, summon-helper suite, diff check,
plugin validation, and real-import-path QML lint. Actual Omarchy has exactly
one Quickshell process, shell ping is `ok`, and fresh warning logs contain only
unrelated UFW kernel blocks. No IPC, desktop-state change, or upstream API
deviation occurred. Ambient selected-date aliasing remains out of scope.

Next bounded unit: isolate the ticker's ambient games from selected-date panel
navigation while preserving one fetch owner and bounded request behavior. Read
the required source-of-truth files and inspect installed Omarchy sources before
editing; add a deterministic selected-date transition test, run all gates,
perform consented runtime verification, update handoff files, and commit
atomically. Stop before performance redesign, geometry, release, or push.

## Latest handoff - 2026-09-05 security and performance review (changes requested)

Reviewed the complete 2026-09-05 change range `8582fa2..3743c1f` against the
public contract, installed Omarchy/Quickshell sources, deterministic gates, and
the live shell. Approval is withheld pending two corrections:

- `components/SourceLinkButton.qml` and `components/TicketStrip.qml` claim an
  exact provider-host launch guard but use substring matching. A URL such as
  `https://example.invalid/espn.com` passes the sink even though upstream
  normalization normally rejects it. The sink must independently parse the
  authority and admit only exact ESPN/NHL/MLB hosts. Align its scheme handling
  with the case-insensitive upstream validator and add executable boundary
  tests rather than source-string assertions alone.
- `SportrayService.ambientGames` aliases the one selected-date `FetchService`.
  While the panel browses another day, every persistent ticker therefore
  switches away from today's ambient live/upcoming/recent-final slate. Preserve
  one fetch owner and existing request bounds, but keep the ticker current-day
  scoped during panel date navigation; add a deterministic selected-date
  transition test.

Sustainment risks that do not independently block this review: the infinite
ticker animation keeps the shell rendering continuously, up to 24 delegates
and 48 logo Images are eagerly instantiated per output, and each minute tick
rebuilds the presentation array. A read-only three-second sample showed the
whole Quickshell process at approximately 4% CPU after its first sample, but no
valid ticker-off baseline was taken, so do not attribute that number solely to
Sportray. Benchmark before broad multi-monitor release and provide a real
reduced-motion/pause route if continued motion remains part of the product.
The Overlay-layer KeyboardPanel dismissal surface also sits above the Top-layer
ticker while the panel is open; verify the intended one-click behavior before
claiming ticker actions remain directly reachable in that state.

The owner's requested main-popout border is already restored: `Panel.qml` no
longer supplies `Border.none()`, and installed
`/usr/share/omarchy/shell/Ui/KeyboardPanel.qml` defaults to a theme-aware popup
border of at least 2 scaled pixels. Preserve this while evaluating any later
left/right full-panel design.

Evidence: clean `main` equals `origin/main`; 269 deterministic JavaScript
checks pass; summon-helper suite passes; `git diff --check` passes; plugin
validation exits 0; real-import-path `qmllint` exits 0 with established
warnings only. Actual Omarchy has one Quickshell process (pid 387125), shell
ping `ok`, bar 1920x26, ticker 1920x32 at y=1048, and no warning-priority
journal entries since the current HEAD loaded. No source, desktop state,
remote, release, tag, or Marketplace change was made during review.

Next bounded unit: correct the exact-host launch sinks with executable tests.
Stop before ambient-date architecture, animation/performance redesign, panel
geometry, release, push, tag, or Marketplace work.

## Latest handoff - 2026-09-05 ticker per-game click proof (runtime-proven, no code change)

With explicit owner consent ("Assist: I'll click on cue" via prompt;
observe-only, no click injection, no rescan/restart), per-game ticker
click routing is now runtime-proven live on the fixed strip:

- Baseline (read-only): one Quickshell instance (pid 387125, unchanged),
  shell ping `ok`, ticker 1920x32 at y=1048, monitor reserve
  `[0,26,0,32]`, normal Sportray polling with no Sportray error,
  exception, or binding-loop warning. Baseline screenshot
  `/tmp/opencode/sportray-ticker-click-baseline.png` shows game text
  with heading breathing room.
- Safe-game click: owner clicked a game segment; a new Chromium window
  `Charlotte vs. Houston (Sep 5, 2026) Live Score - ESPN` appeared on
  workspace 2 (`hyprctl clients`). Safe game opens its provider page:
  PROVEN.
- Chrome click: owner clicked the orange league heading and twice
  reported the scores panel opened. A screenshot of the open panel
  proved unattainable through the Q&A loop for a pre-existing reason:
  the `KeyboardPanel` dismisses on focus loss, so it is already gone by
  the time the owner switches focus to answer. The owner's verbatim
  report ("popup closes when I change focus") confirms the panel opens
  on heading click and that dismissal is focus-out behavior, not a
  ticker defect. Chrome opens the panel: accepted on double owner
  report plus the unchanged `primaryActionRequested` signal path and
  existing fixture/source-assertion coverage.
- Unsafe-game click was not individually targeted live (games without a
  safe URL are not visually distinguishable in the marquee); the neutral
  non-activatable fall-through (`MouseArea` disabled without a safe
  URL, press falls through to the full-strip panel action) remains
  covered by deterministic source assertions.
- Post-click screenshots show headings, sport-emoji separators, and
  live/final game text (MLS `HOU 0 CLT 0 40'`, MLB `BOS 0 BAL 0 Bottom
  4th`, `ATL 2 PHI 4`) rendering with padding. No interaction or
  presentation correction was justified, so no source, README, or
  CHANGELOG change was made.
- Restoration: the panel self-dismissed on focus change (verified
  absent from layers). No theme/bar change and no rescan/restart were
  performed. Residual: the owner answered "Restored" but a read-only
  `hyprctl clients` check still lists the ESPN Charlotte/Houston
  window on workspace 2; the owner closes it at convenience. No push,
  tag, release, or Marketplace work.

Gates without a code change: 269 deterministic JS checks pass, the
summon-helper suite passes, `git diff --check` passes,
`omarchy plugin validate "$PWD"` exits 0, and real-import-path
`qmllint` over every QML file exits 0 with established warnings only.
`docs/upstream-contract.md` remains absent; installed sources
rechecked (Quickshell 0.3.1, `/usr/share/omarchy/shell`): no
Omarchy/Quickshell boundary changed, no deviation to record.

Next bounded unit: verify the residual ESPN proof window is closed,
then take at most one sustainment correction justified by owner
feedback or fresh observed behavior. Stop before provider, cache,
polling, release, push, tag, or Marketplace work.

Owner screenshot proved a bad regression from the clickable-ticker change:
the strip rendered only league headings and separator emoji with no game
text. Root cause was not color: the per-game `MouseArea` used
`anchors.fill: parent` as a direct child of the `gameHit` Row, and
horizontal anchors on a Row child make the Row refuse layout (live
`WARN scene: QML Row ... Row will not function`), collapsing every game
segment. The prior handoff's "layout verified intact" reading of a low-res
crop was wrong; the same crop already showed the bug.

Fix, as one bounded correction:

- `components/TicketStrip.qml`: `gameHit` is now a plain Item sized by an
  inner `gameHitRow` content Row, with the `gameMouse` target filling the
  Item as the Row's sibling. No horizontal anchor lands on a Row child.
  Guarded `openTickerSource` routing, `PointerInteractionPolicy` gating,
  assistive states, separators, gaps, reserved strip, panel border, and
  disabled Calendar are unchanged.
- Heading right padding restored: the pipe cleanup had dropped the old
  `"  |"` suffix, leaving only 3px `xs` Row spacing between a league label
  and its first game. Headings now end with three non-breaking spaces
  (`\u00A0`, trim-proof and font-relative), mirroring the separator
  convention and the `TicketPresentation` text-model prefix.
- `tests/run-js-tests.js` asserts the Item/Row/sibling structure and the
  heading padding; `CHANGELOG.md` notes the fix under Unreleased.

With explicit owner consent per change, rescanned and restarted once per
fix. Live on pid 387125: screenshot shows game text back with heading air
(`MLB` then gapped first game, `BOS 0 BAL 0 Bottom 4th`, `ATL 2 PHI 4`,
`DET 6 CLE 0`, `LAA 3 PIT 0`, `TB 2 TEX 0`), zero `Cannot specify` anchor
warnings from the new shell (only the pre-existing portal-host warning),
summon `ok` / hide exit 0, ticker 1920x32 at y=1048. 269 JS checks,
summon-helper suite, `git diff --check`, plugin validation, and
real-import-path `qmllint` (established warnings only) pass.

Not runtime-proven: per-game click routing (safe game opens provider page;
unsafe game and chrome open the panel) still lacks a click injector in this
environment. No push, tag, release, or Marketplace work.

Next bounded unit: with explicit owner consent, obtain one reliable
per-game click proof (owner-assisted click or an approved injector confined
to the ticker strip). Stop before provider, cache, polling, release, push,
tag, or Marketplace work.

## Latest handoff - 2026-09-05 ticker per-game live pass (partial, no code change)

With explicit owner consent ("Yes, rescan + restart" via prompt), the plugin
was rescanned (`omarchy-shell shell rescanPlugins`, exit 0) and the shell
restarted (`omarchy restart shell`, exit 0). One Quickshell instance
(pid 376872) is healthy, shell ping returns `ok`, live summon returns `ok`
with hide at exit 0, and `hyprctl layers` shows `omarchy-bar` 1920x26 plus
`io.github.joega.sportray-ticker` 1920x32 at y=1048. The 32-pixel reserved
strip, status-aware bounded ordering, shared service ownership, restored
panel border, and disabled Calendar are preserved. No theme or bar change was
made.

Live evidence: `/home/joeg/Pictures/screenshot-2026-09-05_19-55-54.png`
shows the bottom ticker with sport-emoji league headings (MLB, MLS) and
`/home/joeg/Pictures/screenshot-2026-09-05_19-58-27.png` shows the open
scores panel with live MLB rows and scores rendering (BOS 0 / BAL 0,
ATL 2 / PHI 4, DET 6 / CLE 0, LAA 3 / PIT 0, TB 1 / TEX 0). The owner's
mid-pass "scores are missing" report was transient: at capture time the
panel and ticker both render scores, and the journal shows normal polling
(cache-hit timers, fetch process exit 0) with no Sportray error, exception,
or binding-loop warning. No correction was made for the data layer.

Limitation: per-game click routing (safe game opens provider page;
unsafe game and chrome open the panel) is NOT runtime-proven. No click
injector exists in this environment (`ydotool`/`xdotool`/`wlrctl`
absent; `wtype` is keyboard-only; `ts_uinput` is touchscreen replay), and
the marquee targets are small moving hit areas, so no synthetic click was
attempted. Fixture, source-assertion, and validation gates stand in for
target-routing proof. A new live log line appeared after the restart:
`WARN scene: QML Row at TicketStrip.qml[139:11]: Cannot specify ... anchors
for items inside Row. Row will not function.` This is the pre-existing
`anchors.verticalCenter`-inside-`Row` pattern (the prior strip used it
inside the game row; the new nested `gameHit` Row surfaces it at the new
line). Screenshots prove horizontal layout is intact, so no interaction or
presentation correction was made in this unit.

Next bounded unit: with explicit owner consent, obtain one reliable
per-game click proof (owner-assisted click, or an approved click-injection
method confined to the ticker strip), confirm safe-game browser routing vs
unsafe/chrome panel routing, and dispose the Row-anchor warning (fix or
formally accept with evidence). Make at most one interaction or
presentation correction justified by observed behavior. Stop before
provider, cache, polling, release, push, tag, or Marketplace work.

## Latest handoff - 2026-09-05 ticker per-game provider links

With explicit owner confirmation (approved "Yes, make games clickable" via
prompt before any source change), individual bottom-ticker games are now
activatable:

- `model/TicketPresentation.js` exposes a `safeGameUrl` helper mirroring the
  `GameModel.safeGameUrl` allowlist (HTTPS plus exact ESPN/NHL/MLB hosts) and
  stamps each ticker item with `sourceUrl` (`""` for missing, non-HTTPS, or
  untrusted links). The game segment text, ordering, bounds, sport-emoji
  separators, and pipe-free copy are unchanged.
- `components/TicketStrip.qml` wraps each game segment in a `gameHit` target
  with its own `MouseArea` (enabled only when the game has a safe URL) and an
  assistive press action; both open that game's URL through a new guarded
  `openTickerSource` sink matching `SourceLinkButton.openSource`
  (`omarchy-launch-browser` after the HTTPS + provider-host check). The
  full-strip `stripMouse` moved below the scrolling content in stacking order
  so game presses are grabbed above while separator, heading, gap, status,
  and unsafe-game clicks fall through to the existing panel-open action,
  additionally gated by `PointerInteractionPolicy.allowsRowActivation` over a
  shared `gameActionPressed` flag mirroring the GameRow pattern. The 32-pixel
  reserved strip, 56-pixel group gaps, three-space emoji padding, accent
  edge, marquee animation, and root panel-open accessible action are
  unchanged.
- `fixtures/bar-presentation/ticket-strip.json` carries safe ESPN links on
  the live and scheduled games and an `http://evil.example.com` link on the
  recent final; deterministic coverage asserts per-game `sourceUrl` routing,
  rejection of HTTP/untrusted/malformed/missing links, the sink guard and
  `omarchy-launch-browser` route, nested-pointer gating, per-game
  Button/StaticText assistive states, stacking order, and the preserved
  panel action and geometry.

All 269 deterministic JavaScript checks pass (267 prior plus the two new
ticker-link tests), plus the summon-helper suite, `git diff --check`,
`omarchy plugin validate "$PWD"` (exit 0), and real-import-path `qmllint`
over every QML file (exit 0, established warnings only). On actual Omarchy,
one Quickshell instance (pid 360277) remained healthy, shell ping returned
`ok`, live summon returned `ok` with hide at exit 0, and the shell log showed
normal Sportray polling with no Sportray error, exception, or binding-loop
warning. The live shell was not rescanned or restarted in this unit (no owner
consent for a desktop-state change), so it still runs the pre-change strip:
the new targets are verified by fixture, source-assertion, and validation
gates, and a live click/visual pass remains available with owner consent for
a rescan/restart. Bottom-edge reserved space, status-aware bounded ordering,
shared service ownership, the restored panel border, sport-emoji separators,
and disabled Calendar behavior are preserved. No provider, cache, polling,
release, push, tag, or Marketplace work was performed.
`docs/upstream-contract.md` remains absent; no Omarchy/Quickshell boundary
changed (the `Quickshell.execDetached` browser route and
`PointerInteractionPolicy` gate were already in use elsewhere in the tree).

Next bounded unit: with explicit owner consent for a plugin rescan and shell
restart, perform one live pass of the new per-game ticker targets (safe game
opens its provider page, unsafe game and chrome open the panel) plus the
still-pending sport-emoji separator visual check, then restore nothing (no
theme or bar change involved). Make at most one interaction or presentation
correction justified by observed behavior. Stop before provider, cache,
polling, release, push, tag, or Marketplace work.

## Latest handoff - 2026-09-05 ticker sport-emoji separators

Owner sustained-use feedback: ticker spacing is fine; the requested
separator cleanup was replacing the orange bullet (`•`) same-game separators
with the sport emoji of the next listed game (for example, a soccer ball
between soccer games), then dropping both `|` pipes from the ticker line
after seeing a screenshot (`⚽ MLS | NE @ LA | 9:30 PM`). Implemented as the
single focused separator adjustment for this unit:

- `model/TicketPresentation.js` now prefixes same-league continuations with
  the current game's league emoji (`"   " + emoji + "   "`, preserving the
  three-space padding) instead of `"   •   "`, and exposes
  `separatorEmoji` on each ticker item. League-group headings drop the
  trailing pipe (`emoji + "  " + label` with spacing), and game segments
  join matchup and detail with spaces instead of `"  |  "`.
- `components/TicketStrip.qml` renders the non-heading separator from
  `separatorEmoji` (falling back to `leagueEmoji`, then `•`), the heading
  without its pipe, and the detail without its leading pipe, keeping the
  accent color, row spacing, 56-pixel group gap, 32-pixel reserved strip,
  and full-strip panel-open action unchanged.
- `fixtures/bar-presentation/ticket-strip.json` expects the soccer-ball
  separator between the two MLS games and no pipes anywhere; deterministic
  coverage asserts `separatorEmoji === "⚽"` on the continued game and no
  `•` or `|` in ticker text. The source-driven strip test asserts
  `separatorEmoji` routing and rejects the old bullet and pipe literals.

All 267 deterministic JavaScript checks pass, plus the summon-helper suite,
`git diff --check`, `omarchy plugin validate "$PWD"` (exit 0), and
real-import-path `qmllint` over every QML file (exit 0, established warnings
only). On actual Omarchy, one Quickshell instance (pid 360277) remained
healthy, shell ping returned `ok`, live summon returned `ok` with hide at
exit 0, and the shell log showed normal Sportray polling with no Sportray
error, exception, or binding-loop warning. The live shell was not rescanned
or restarted in this unit, so the new separators are verified by fixture,
source-assertion, and validation gates; a live visual pass remains available
with owner consent for a shell restart. Bottom-edge reserved space,
status-aware bounded ordering, shared service ownership, the restored panel
border, and disabled Calendar behavior are preserved. No provider, cache,
polling, release, push, tag, or Marketplace work was performed.
`docs/upstream-contract.md` remains absent; no Omarchy/Quickshell boundary
changed (no new imports, components, or APIs).

Next bounded unit: pending explicit owner confirmation, make individual
ticker games clickable so activating a specific game opens its provider game
page (ESPN gamecast, MLB.com gameday, or NHL.com gamecenter) through the
existing guarded source route, while preserving the full-strip panel-open
action, nested-pointer safety, reserved space, ordering, service ownership,
panel border, and disabled Calendar. Stop before provider, cache, polling,
release, push, tag, or Marketplace work.

## Latest handoff - 2026-09-05 light-theme visual verification (pass, no fix)

With explicit owner consent (approved "Catppuccin Latte" via prompt before
any desktop change), the bottom ticker and restored main-panel border
received one actual light-theme visual pass, and the exact original theme
was restored afterward. No theme-adaptive correction was justified, so no
code, README, or CHANGELOG change was made.

- Pre-switch state recorded: `omarchy theme current` = Matte Black
  (`theme.name` = `matte-black`), background = Ship At Sea staged at
  `.../current/theme/backgrounds/0-ship-at-sea.jpg`
  (md5 `a6494e5349ce1ff15565e5b8733f0d24`, stock source present), one
  Quickshell instance (pid 360277).
- `omarchy theme set "Catppuccin Latte"` applied (background Color Fade);
  the same shell PID survived, shell ping returned `ok`, and summon IPC
  returned `ok` with no rescan or restart required beyond the theme switch.
- Light-theme evidence: `/tmp/opencode/screenshot-2026-09-05_19-33-49.png`
  shows live MLB ticker games plus the open bordered scores panel;
  `/tmp/opencode/screenshot-2026-09-05_19-34-46.png` shows the scrolled
  upcoming-games ticker segment with live data advanced (DET/CLE Middle
  6th), proving animation and refresh on the light theme. Crops
  `ticker-crop-1.png` (live) and `ticker-crop-2.png` (upcoming) show
  full-color team logos, legible dark-on-light text, gray secondary status
  detail, and spaced blue bullet separators. `hyprctl layers` listed
  `omarchy-bar` 1920x26, `io.github.joega.sportray-ticker` 1920x32 at
  y=1048, and `omarchy-keyboard-panel` while open; monitors reserved
  `[0,26,0,32]`. The journal showed normal Sportray polling/fetch and no
  Sportray error, exception, or binding-loop warning (only pre-existing
  XKB/XCompose and portal host warnings). Hide IPC succeeded and the
  ticker persisted after the panel closed.
- Findings: no contrast, emoji/tofu, logo, separator-spacing, or border
  defect reproduced. A first-glance impression that upcoming logos looked
  muted was checked against `TicketStrip.qml:166` (the only ticker
  opacity, 0.82 on live status text; no logo dimming) and the crops show
  colored crests, so no correction was made. The open panel rendered as a
  distinct light card (date carousel, Following live rows, red scores,
  Watch/ESPN actions) framed by the restored `KeyboardPanel` default
  border. Emoji league headings were not mid-frame in either capture, but
  no missing-glyph boxes appeared anywhere.
- Restoration: `omarchy theme set "Matte Black"` plus `omarchy theme bg
  set` on the stock `0-ship-at-sea.jpg` restored `theme.name` =
  `matte-black`, background Ship At Sea at the identical staged path with
  matching md5, the same Quickshell PID, shell ping `ok`, and summon/hide
  IPC healthy on the restored theme.

Next bounded unit: collect owner feedback from sustained bottom-ticker use
and make at most one focused presentation adjustment (speed, density,
separators, or contrast). Preserve bottom-edge reserved space,
status-aware bounded ordering, shared service ownership, the existing
scores panel with its restored border, and disabled Calendar behavior.
Stop before provider, cache, polling, release, push, tag, or Marketplace
work.

## Latest handoff - 2026-09-05 review hardening and panel border

Owner requested a full review pass plus restoring the main panel border. This
unit implements the review findings as one bounded slice:

- `SourceLinkButton.openSource()` now rejects non-HTTPS URLs and requires an
  ESPN/NHL/MLB host at the sink, matching the `GameModel.safeGameUrl`
  allowlist upstream. `EspnProvider.teamLink()` uses `safeGameUrl` instead of
  the broad `safeUrl`, and the now-unused `safeUrl` helpers in both providers
  were removed.
- Provider-derived text in `TicketStrip` delegates and `GameRow` labels uses
  `Text.PlainText`. `GameRow` logos decode at 2x display size with `cache: true`.
- `FetchService` coalesces the per-league signal bursts (games, loading, stale,
  error, timestamps) through `requestAggregateUpdate()` with a `Qt.callLater`
  guard; explicit user actions (enable/date/open) still recompose directly.
- `FavoritePresentation` normalizes once per sort/select pass via
  `isFavoriteGameNormalized`/`categoryNormalized`/`compareEntriesNormalized`;
  `PanelPresentation.annotate()` normalizes once per game instead of three times.
- The main scores panel no longer overrides `borderSpec: Border.none()`, so the
  installed `KeyboardPanel` default popup border frames the card again. Bar
  attachment (margin/gap 0, centerOnBar) is unchanged.

All 267 deterministic JavaScript checks pass, including a new bounded test for
the sink guard, logo bounds, aggregate coalescing, normalized favorite checks,
and border restoration. The summon-helper test, `git diff --check`, plugin
validation (exit 0), and real-import-path `qmllint` (exit 0, established
warnings only) pass. On actual Omarchy, one Quickshell instance remained
healthy, shell ping returned `ok`, and summon/hide IPC for the linked checkout
both returned `ok` with the border change live.

Next bounded unit: with explicit owner consent before changing their desktop
theme, perform one actual light-theme visual pass of the ticker and panel
border, then restore the original theme afterward. Make at most one
theme-adaptive presentation fix if a defect is reproduced. Stop before
provider, selection, animation, release, push, tag, or Marketplace work.
